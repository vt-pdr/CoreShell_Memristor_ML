...
# =====================================================================
# 4. STAGE 3: MULTI-PARADIGM MODEL TRAINING (XGB + ET + KNN + MLP)
# =====================================================================
print("\n[STAGE 3] Training 4-Model System (Gradient Trees + Bagged Trees + KNN + MLP Neural Net)...")
...
# 3. KNN Distance Regressor Pipeline
...
# 4. Multi-Layer Perceptron (MLP Neural Network) Pipeline
...
# Test Performance
print(f" -> XGBoost    : R² = {r2_score(y_test, xgb_model.predict(X_test)):.4f}")
print(f" -> ExtraTrees : R² = {r2_score(y_test, et_model.predict(X_test)):.4f}")
print(f" -> KNN        : R² = {r2_score(y_test, knn_pipeline.predict(X_test)):.4f}")
print(f" -> MLP Net    : R² = {r2_score(y_test, mlp_pipeline.predict(X_test)):.4f}")

# =====================================================================
# 5. STAGE 4: OUT-OF-FOLD CROSS-VERIFICATION & CONSENSUS
# =====================================================================
print("\n[STAGE 4] Performing 4-Way Out-of-Fold Cross-Verification...")
...
oof_knn = cross_val_predict(knn_pipeline, X_elite, y_elite, cv=cv, n_jobs=-1)
oof_mlp = cross_val_predict(mlp_pipeline, X_elite, y_elite, cv=cv, n_jobs=-1)

elite_df['XGB_Quality'] = oof_xgb
elite_df['ET_Quality'] = oof_et
elite_df['KNN_Quality'] = oof_knn
elite_df['MLP_Quality'] = oof_mlp

# 4-Model Coefficient of Variation Dispersion
preds_matrix = np.column_stack([oof_xgb, oof_et, oof_knn, oof_mlp])
means = np.mean(preds_matrix, axis=1)
stds = np.std(preds_matrix, axis=1)
cv_dispersion = np.where(means > 0, stds / means, 0)

elite_df['Verification_Agreement_%'] = np.clip(100 * (1 - cv_dispersion), 0, 100)

# Multi-Paradigm Consensus (35% XGB / 25% ET / 20% MLP / 20% KNN)
elite_df['Consensus_Quality'] = (
    (0.35 * oof_xgb) + (0.25 * oof_et) + (0.20 * oof_mlp) + (0.20 * oof_knn)
)
...
joblib.dump(xgb_model, os.path.join(models_dir, "xgb_model_v2.joblib"))
joblib.dump(et_model, os.path.join(models_dir, "extra_trees_model_v2.joblib"))
joblib.dump(knn_pipeline, os.path.join(models_dir, "knn_pipeline_v2.joblib"))
joblib.dump(mlp_pipeline, os.path.join(models_dir, "mlp_pipeline_v2.joblib"))
joblib.dump(scaler_global, os.path.join(models_dir, "scaler_global_v2.joblib"))

output_file = os.path.join(data_dir, "phase4b_multiparadigm_memristors.csv")
export_cols = [
    'Core_Formula',
    'Shell_Formula',
    'Consensus_Quality',
...
print(f"SUCCESS! Multi-Paradigm candidates saved to: '{output_file}'")