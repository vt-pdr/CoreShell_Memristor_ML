import sys
# Print this IMMEDIATELY before doing any heavy lifting
print("Initializing script... (Loading Materials Project physics libraries, this takes 10-30 seconds)...")
sys.stdout.flush()
...
# =====================================================================
# 2. OFFICIAL MP-API HARVESTING (Split Endpoints)
# =====================================================================
def fetch_all_stable_semiconductors(limit=MAX_MATERIALS):
    print("\nScanning Materials Project via MPRester for dielectric and formation energy data...")
    data_records = []
    ...
    try:
        with MPRester(MP_API_KEY) as mpr:
            ...
            # FIXED: 'dielectric' is no longer allowed here. We request 'has_props' instead.
            ...
            # Find which materials actually have dielectric data available
            print("  -> Cross-referencing real dielectric tensors from the dedicated endpoint...")
            mp_ids_with_diel = [
                str(doc.material_id) for doc in top_docs 
                if doc.has_props and "dielectric" in doc.has_props
            ]
            
            # Batch fetch the actual dielectric data
            dielectric_map = {}
            if mp_ids_with_diel:
                try:
            ...
            for doc in top_docs:
                ...
                # Pull the real value we mapped, or fallback ONLY if physically unavailable
                eps_static = dielectric_map.get(mp_id_str, 10.0) 
...
# =====================================================================
# 3. NANOSCALE CORRECTION (Brus Equation)
# =====================================================================
